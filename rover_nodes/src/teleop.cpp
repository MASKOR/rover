...
class Teleop {
 public:
      ...
      last_joy_msg_received_(0)
  {
    ...
    nh_.param<int>("deadman_button2", deadman_button_, 6);
    nh_.param<int>("speed_button", speed_button_, 0);
    nh_.param<int>("throttle_axis" , throttle_axis_ , 1);
    nh_.param<int>("steering_axis" , steering_axis_ , 2);
    ...
    joy_sub_ = nh_.subscribe(joy_topic_, 1, &Teleop::_joy_cb, this); 
    cmd_vel_pub_ = nh_.advertise<geometry_msgs::Twist>(cmd_vel_topic_, 1);

    ROS_INFO("***** LOADING TELEOP *****");
    ...
    ROS_INFO("deadman_button2: %d", deadman_button2_);
 ...
 private:
  ...
  int deadman_button_, deadman_button2_, speed_button_;
  ...
  bool deadman, deadman2, speed, arming;
  ...
  void _joy_cb(const sensor_msgs::Joy::ConstPtr &msg) {
    ...
    //ROS_INFO("Deadman-Button: %d",  msg->buttons[deadman_button_]);
      ...
      deadman2 = msg->buttons[deadman_button2_];
  ...
  void _publish() {
    ...
    **/
    ...
    if(ros::Time::now().toSec() - last_joy_msg_received_.toSec() > joy_msg_timeout_) {
      ROS_WARN_NAMED("teleop","[teleop]: no joy message received for %0.1fs, setting throttle = 0.0 and yaw = 0.0", joy_msg_timeout_);
      yaw_ = 0.0;
      throttle_ = 0.0;
    }
    else {
	...
	cmd_vel.linear.x = -0.5 * throttle_ + 0.5;
...
if (tmp_a_scale > 1.0){
	...
	if (deadman || deadman2){
	  ...
	  cmd_vel.linear.x = -0.5 * throttle_ + 0.5;
          ...
          //reverse
          if(deadman2)
            cmd_vel.linear.x = cmd_vel.linear.x * (-1.0f);
